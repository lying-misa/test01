...
class CorrectionController extends Controller
{
    ...
    function Save(){
       ...
       // if(!$info['Reason']) $this->error("Reason为空！");
        ...
        $res2 = $KnowledgeQs->where('ID='.$QID)->save(array("Answer"=>$infos['Answer'],'Analysis'=>$infos['Reason']));
        if($res && $res2){
            $this->success('设置成功！');
        }else{
            $this->error('设置失败！');
