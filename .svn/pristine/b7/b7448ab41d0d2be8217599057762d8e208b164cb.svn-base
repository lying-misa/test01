...
class OldQuestionsController extends Controller
{
    ...
    function index()
    {
        ...
        $Type = I('Type','', 'trim');
        if($Type!=NULL && $Type!="" && in_array($Type, array(1, 2, 3))){
            $map['Type'] = $Type;
        }
    ...
    function ReadExcel($file,$info){
        ...
        for($currentRow = 2,$i=1; $currentRow <= $allRow; $currentRow++,$i++) {
            ...
            $arr[$i]['OldExamID'] = $info['OldExamID'];
    ...
    function UploadExcel()
    {
        ...
        $base['OldExamID'] = I('OldExamID', '', 'trim');
        $returnStr['status'] = 0;
        header("Content-Type:text/html;charset = utf-8");
    ...
    function Arr2ImportQuestions($arr){
        ...
        for(;$i<=$count;$i++) {
            ...
            if($arr[$i]['Type']==3){
                ...
                $arr1[] = $arr[$i]['F'];
                $data['Answer'] = $arr[$i]['G'];
                $data['Analysis'] = $arr[$i]['H']?$arr[$i]['H']:"";
                $data['Degree'] = $arr[$i]['J']?$arr[$i]['J']:"";
            }elseif($arr[$i]['Type'] == 1){
                $arr1[] = $arr[$i]['B'];
                $arr1[] = $arr[$i]['C'];
                $arr1[] = $arr[$i]['D'];
                $arr1[] = $arr[$i]['E'];
                $data['Answer'] = $arr[$i]['F'];
                $data['Analysis'] = $arr[$i]['G']?$arr[$i]['G']:"";
                $data['Degree'] = $arr[$i]['H']?$arr[$i]['H']:"";
            }else{
                $data['Answer'] = 'X';
                $data['Analysis'] = $arr[$i]['B']?$arr[$i]['B']:"";
                $data['Degree'] = $arr[$i]['C']?$arr[$i]['C']:"";
            }
