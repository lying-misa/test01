...
class PracticeController extends Controller
{
    ...
    */
    ...
    function MakeOldScore()
    {
        ...
        $answers= I('Reply','', 'trim');
        //$Reply = $this->Num2Char($Reply);
       // $answers = '[["B",404],["B",405],["C",518]]';
       // $sd = array(
        //    array('B',404),
         //   array('B',405),
         //   array('C',518),
       // );
       // $ressx = json_encode($sd);
        $answersarr = json_decode($answers);
        $count = count($answersarr);
        $rightNum = 0;
        unset($_SESSION['User']['oldexam_practice']);
        for($i=0;$i<$count;$i++) {
            //答案拆分 --匹配多选单选情况
            $ID = array_pop($answersarr[$i]);
            //将答案转换为字母
            ...
            $data['UserID'] = $_SESSION['User']['UserID'];
            $data['Answer'] = implode(',', $op);
            $ress = $Questions->Field('Answer,Type')->where($where)->find();
            $data['TrueAnswer'] = $ress['Answer'];
            $data['Type'] = $ress['Type'];
            if ($data['TrueAnswer'] == $data['Answer']) {
                $data['IsTrue'] = 1;
                $rightNum = $rightNum + 1;
                ...
                $data['IsTrue'] = 0;
            $_SESSION['User']['oldexam_practice'][$i]['QID'] = $data['QID'];
            $_SESSION['User']['oldexam_practice'][$i]['Type'] = $ress['Type'];
            $_SESSION['User']['oldexam_practice'][$i]['Answer'] = $answersarr[$i];
            $_SESSION['User']['oldexam_practice'][$i]['TrueAnswer'] = $data['TrueAnswer'];
            $_SESSION['User']['oldexam_practice'][$i]['IsTrue'] = $data['IsTrue'];
            $data = array();
        }
        //var_dump($_SESSION['User']['oldexam_practice']);
        if($i == $count){
            $info['ExamID'] = $data['ExamID'] ? $data['ExamID'] : 1;
            $info['UserID'] = $_SESSION['User']['UserID']?$_SESSION['User']['UserID']:"1";
            $info['Reply'] = $answers;
            $info['CreateDate'] = time();
            $ExamRecord = M('OldExamRecord');
            $info['Score'] = $rightNum;
            $res = $ExamRecord->add($info);
            if ($res) {
                $returnStr['status'] = 1;
        ...
        $_SESSION['User']['oldexam_practicefenshu'] = $rightNum;
       // $this->assign('list',$_SESSION['User']['oldexam_practice']);
       // $this->assign("fenshu", $rightNum);
        return $this->ajaxReturn($returnStr);
       // $this->display();
       // header("Location: /mobile/Practice/OldExamSheet");
    }
    /**
     * 提交题目，答案列表
     * author: lying
