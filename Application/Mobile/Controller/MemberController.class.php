...
class MemberController extends Controller
{
    function IsLogin(){
        if(!isset($_SESSION['User']['UserID']) && $_SESSION['User']['UserID'] == NULL && $_SESSION['User']['UserID'] == '' &&
            ...
            $this->error("请登录","mobile/Member/login");
        }
    }
	/**
	* 用户登录
	...
	**/
     function  Reg()
    {
		...
		if(IS_POST){
			...
			if(!$data['Tel'])  {$returnStr['errormsg'] = '参数丢失：手机号为空';$this->error($returnStr['errormsg'],'/mobile/member/reg');}
			if(!$data['Password'])  {$returnStr['errormsg'] = '参数丢失：密码为空';$this->error($returnStr['errormsg'],'/mobile/member/reg');}
			if(!$data['Code'])  {$returnStr['errormsg'] = '参数丢失：验证码为空';$this->error($returnStr['errormsg'],'/mobile/member/reg');}
			if($data['Code'] != $_SESSION['Code'] && $_SESSION['Code']!='' && $_SESSION['Code']!=null){
				$returnStr['errormsg'] = '参数丢失：验证码错误';$this->error($returnStr['errormsg'],'/mobile/member/reg');
			}else{
				if(!$this->IsHaveUser('Tel='.$data['Tel'])){
					...
					if($res){
						...
						$_SESSION['User']['Tel'] = $res['Tel'];
						//$this->ajaxReturn($returnStr);
                        $this->success($returnStr['errormsg'],'/mobile');
					}else{
						$returnStr['errormsg'] = '添加失败！';
						//$this->ajaxReturn($returnStr);
                        $this->error($returnStr['errormsg'],'/mobile/member/reg');
					}

				}else{
					$returnStr['errormsg'] = '手机号已存在';$this->error($returnStr['errormsg'],'/mobile/member/reg');
				}
			}
		}else
			$this->display();
     ...
     */
     function  setCollect(){
         $this->IsLogin();
			...
			$ifcollect=$user_collect->where($data)->find();
			if($ifcollect){
